...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error?.code === 'P1001' || error?.message?.includes('Can\'t reach database')) {
      console.warn('[API] Error de conexión a la base de datos - servicio temporalmente no disponible');
      return NextResponse.json(
        { error: 'Servicio temporalmente no disponible. Reintentando...' },
    ...
    if (error?.code?.startsWith('P')) {
      console.error('[API] Error de base de datos:', error.code, error.message);
      return NextResponse.json(
