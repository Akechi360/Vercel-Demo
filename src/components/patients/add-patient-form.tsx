...
export function AddPatientForm({ onSuccess }: AddPatientFormProps) {
  ...
  // Listen for patient deletion events to refresh selectable users
  useEffect(() => {
    const handlePatientDeleted = async (event: CustomEvent) => {
      console.log('🔄 Patient deleted event received, refreshing selectable users...', event.detail);
      try {
        const usersData = await listSelectablePatientUsers();
        setSelectableUsers(usersData);
        console.log('✅ Selectable users refreshed after patient deletion:', usersData);
      } catch (error) {
        console.error('❌ Error refreshing selectable users after patient deletion:', error);
      }
    };
