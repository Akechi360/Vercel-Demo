...
export async function createTestAffiliation(): Promise<any> {
  try {
    ...
    // Skip database availability check for now to avoid issues
    console.log('🔍 Attempting direct database connection...');
    
    const prisma = getPrisma();
    
    // First, get a company and user with error handling
    let companies, users;
    try {
      console.log('🔍 Querying companies...');
      companies = await prisma.company.findMany({ take: 1 });
      console.log('🔍 Querying users...');
      users = await prisma.user.findMany({ take: 1 });
    } catch (dbError) {
      console.error('❌ Error querying database:', dbError);
      throw new Error(`Error al consultar la base de datos: ${dbError instanceof Error ? dbError.message : 'Error desconocido'}`);
    }
    ...
    if (companies.length === 0) {
      ...
      throw new Error('No hay empresas en la base de datos. Ve a "Empresas" y crea una empresa primero.');
    }
    
    if (users.length === 0) {
      console.error('❌ No users found in database');
      throw new Error('No hay usuarios en la base de datos. Ve a "Usuarios" y crea un usuario primero.');
    }
    
    console.log('✅ Found company:', companies[0].id, 'and user:', users[0].id);
    
    // Create the test affiliation
    ...
    console.log('✅ Test affiliation created successfully:', testAffiliation.id);
    return testAffiliation;
  } catch (error) {
    console.error('❌ Error creating test affiliation:', error);
    // Return a more user-friendly error message
    if (error instanceof Error) {
      throw new Error(`Error al crear afiliación de prueba: ${error.message}`);
    }
    throw new Error('Error desconocido al crear afiliación de prueba.');
  }
}
